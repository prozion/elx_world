...
(define public? group?)
;
(define (vk-members-number entity)
...
(define-catch (cache-posts-by-uid
            ...
            #:request-limit (request-limit #f)
            )
    (for/fold
      ((res_chunk (hash)) (buffer_count 0) (count 0))
      ((uid uids))
      #:break (and request-limit (>= count request-limit))
            ...
            (let ((result (hash-union res_chunk (hash uid posts)))
              ...
              (cond
                ((>= buffer_count cache-write-frequency)
                  ...
                  (values (hash) 0 (+ count buffer_count)))
                (else
                  (values result buffer_count count)))))
          (else
            (values res_chunk buffer_count count)))))
    #t)
